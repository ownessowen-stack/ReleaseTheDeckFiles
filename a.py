...
Fixed: No white square at startup, non-blocking TDP changes
"""
    ...
    def get_focused_app_info(self):
        ...
        try:
            ...
            if name in skip_processes:
                logging.debug(f"Skipping system process: {name}")
                return None, None, None
...
class PowerModeManager:
    ...
    def __init__(self):
        ...
        # Get current settings on startup
    ...
    def get_current_settings(self):
        ...
        try:
            ...
            if result.stdout:
                ...
                # Get mode ID from the last line
                lines = output.split('\n')
    ...
    def set_mode_by_id_async(self, mode_id, callback=None):
        """Set power mode in a separate thread to avoid freezing"""
        def run():
            try:
                result = subprocess.run(['sudo', 'nvpmodel', '-m', str(mode_id)],
    ...
    def set_fan_async(self, fan_mode, callback=None):
        ...
        def run():
            try:
                result = subprocess.run(['sudo', 'nvpmodel', '-d', fan_mode],
                                       check=True, capture_output=True, timeout=5)
...
class QuickAccessMenu:
    def __init__(self, root, app_suspender, power_manager):
        ...
        # Hide window until fully initialized to prevent white square
        self.root.withdraw()
        ...
        # Position in top-right corner - FULL HEIGHT
        ...
        # Force window to top after creation and show it
        self.root.after(100, self.show_window_and_force_top)

        logging.info("QuickAccessMenu initialized")

        # Show notification if game suspended
        if self.app_suspender.suspended_name:
            self.show_notification()

    def show_window_and_force_top(self):
        """Show window after initialization and force it to top"""
        self.root.deiconify()  # Show the window
        self.force_to_top()
        self.root.focus_force()

    def force_to_top(self):
        """Force window to top and ensure position"""
        try:
            self.root.lift()
            self.root.focus_force()
            self.root.attributes('-topmost', True)
    ...
    def show_notification(self):
        """Show notification that game is suspended"""
        try:
            subprocess.run(['notify-send',
                          'Quick Access',
                          f'{self.app_suspender.suspended_name} suspended',
    ...
    def quit_menu(self):
        """Quit the menu and resume the game"""
        ...
        if self.app_suspender.suspended_pids:
            for pid in self.app_suspender.suspended_pids:
                try:
                    ...
                    logging.info(f"Killed game PID {pid}")
                except:
                    pass
        self.root.quit()

    def update_display_dimensions(self):
        """Position window in top-right corner - FULL HEIGHT"""
    ...
    def create_main_container(self):
        """Create the main window with left sidebar and right content"""
        # Main frame
    ...
    def create_sidebar(self):
        """Create left sidebar with category buttons and game info"""
        sidebar = tk.Frame(self.inner_frame, bg=self.colors['card_bg'], width=200)
        sidebar.pack(side=tk.LEFT, fill=tk.Y, padx=0, pady=0)
        sidebar.pack_propagate(False)

        # Header
        tk.Label(sidebar, text="QUICK ACCESS", font=('Arial', 14, 'bold'),
        ...
        for cat_id, name, color in categories:
            btn = tk.Button(sidebar, text=name, font=('Arial', 12),
        ...
        tk.Label(game_frame, text=f"PID: {pid_text}", font=('Arial', 9),
                bg=self.colors['card_bg'], fg=self.colors['fg']).pack(anchor='w')

        tk.Label(game_frame, text="STATUS: SUSPENDED", font=('Arial', 9, 'bold'),
                bg=self.colors['card_bg'], fg=self.colors['warning']).pack(anchor='w', pady=(2, 5))

        # Game action buttons
        btn_frame = tk.Frame(game_frame, bg=self.colors['card_bg'])
        btn_frame.pack(fill=tk.X, pady=5)

        resume_btn = tk.Button(btn_frame, text="RESUME", font=('Arial', 10, 'bold'),
        ...
        kill_btn = tk.Button(btn_frame, text="KILL", font=('Arial', 10, 'bold'),
    ...
    def create_content_area(self):
        """Create right content area that changes based on category"""
    ...
    def switch_category(self, category_id):
        """Switch the content area to show selected category"""
    ...
    def create_system_content(self):
        """Create system controls (brightness + volume)"""
        # Title
        title = tk.Label(self.content_scrollable, text="SYSTEM CONTROLS", font=('Arial', 18, 'bold'),
        ...
        tk.Label(brightness_card, text="BRIGHTNESS", font=('Arial', 14, 'bold'),
        ...
        for label, level in [("Night", 20), ("Indoor", 60), ("Full", 100)]:
            btn = tk.Button(preset_frame, text=label, font=('Arial', 12),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
        ...
        tk.Label(volume_card, text="VOLUME", font=('Arial', 14, 'bold'),
                bg=self.colors['card_bg'], fg=self.colors['accent']).pack(anchor='w', padx=20, pady=(20, 10))
        ...
        self.mute_btn = tk.Button(control_frame, text="MUTE", font=('Arial', 12, 'bold'),
                                  bg=self.colors['button_bg'], fg=self.colors['fg'],
                                  bd=0, padx=15, pady=8, cursor='hand2',
                                  activebackground=self.colors['button_hover'],
                                  command=self.toggle_mute)
        self.mute_btn.pack(side=tk.LEFT, padx=3)
        self.all_widgets.append(self.mute_btn)

        for level, label in [(50, "50%"), (100, "100%")]:
            btn = tk.Button(control_frame, text=label, font=('Arial', 12),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
                           bd=0, padx=15, pady=8, cursor='hand2',
                           activebackground=self.colors['button_hover'],
    ...
    def create_tdp_content(self):
        """Create TDP and Fan controls - non-blocking buttons"""
        # Title
        title = tk.Label(self.content_scrollable, text="TDP & FAN CONTROL", font=('Arial', 18, 'bold'),
                        bg=self.colors['bg'], fg=self.colors['accent'])
        title.pack(pady=(0, 20))
        ...
        tk.Label(tdp_card, text="TDP MODE", font=('Arial', 14, 'bold'),
                bg=self.colors['card_bg'], fg=self.colors['accent']).pack(anchor='w', padx=20, pady=(20, 10))
        ...
        for mode_id, name in tdp_modes[:4]:
            btn = tk.Button(row1, text=name, font=('Arial', 11),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
                           bd=0, padx=10, pady=6, cursor='hand2',
                           activebackground=self.colors['button_hover'],
        ...
        for mode_id, name in tdp_modes[4:]:
            btn = tk.Button(row2, text=name, font=('Arial', 11),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
                           bd=0, padx=10, pady=6, cursor='hand2',
                           activebackground=self.colors['button_hover'],
                           command=lambda m=mode_id, n=name: self.set_tdp_mode_async(m, n))
        ...
        tk.Label(fan_card, text="FAN MODE", font=('Arial', 14, 'bold'),
                bg=self.colors['card_bg'], fg=self.colors['accent']).pack(anchor='w', padx=20, pady=(20, 10))
        ...
        for fan_mode in ["Console", "Handheld", "Cool"]:
            btn = tk.Button(fan_row, text=fan_mode, font=('Arial', 11),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
                           bd=0, padx=10, pady=6, cursor='hand2',
                           activebackground=self.colors['button_hover'],
    ...
    def set_tdp_mode_async(self, mode_id, name):
        """Set TDP mode in background thread, update UI when done"""
        # Disable the button temporarily? Optional
        def callback(success, new_name):
            if success:
                ...
                try:
                    subprocess.run(['notify-send', 'Error', f'Failed to switch to {name}',
                                  '-t', '3000'], stderr=subprocess.DEVNULL)
                except:
                    pass
    ...
    def set_fan_mode_async(self, fan_mode):
        """Set fan mode in background thread, update UI when done"""
        def callback(success, new_mode):
            if success:
                ...
                try:
                    subprocess.run(['notify-send', 'Error', f'Failed to switch to {fan_mode} fan',
                                  '-t', '3000'], stderr=subprocess.DEVNULL)
                except:
                    pass
    ...
    def create_scripts_content(self):
        ...
        title = tk.Label(self.content_scrollable, text="SCRIPTS", font=('Arial', 18, 'bold'),
                        bg=self.colors['bg'], fg=self.colors['accent'])
        title.pack(pady=(0, 20))
        ...
        for name, cmd in scripts:
            btn = tk.Button(scripts_card, text=name, font=('Arial', 13),
                           bg=self.colors['button_bg'], fg=self.colors['fg'],
    ...
    def create_power_content(self):
        """Create power controls content"""
        title = tk.Label(self.content_scrollable, text="POWER", font=('Arial', 18, 'bold'),
                        bg=self.colors['bg'], fg=self.colors['accent'])
        title.pack(pady=(0, 20))
        ...
        for name, cmd, color in power_options:
            btn = tk.Button(power_card, text=name, font=('Arial', 13),
                           bg=color, fg=self.colors['fg'],
                           bd=0, padx=20, pady=12, cursor='hand2',
                           activebackground=self.colors['button_hover'],
                           command=lambda c=cmd: self.run_command(c))
...
def main():
    ...
    # Create app suspender and suspend the game
    suspender = AppSuspender()
    suspended = suspender.suspend_focused_app()
